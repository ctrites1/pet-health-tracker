...
export const petsRoute = new Hono()
  .get('/', getUser, async c => {
    try {
      ...
      const pets = await db.select().from(petsTable).where(eq(petsTable.ownerId, user.id));
  ...
  .get('/:id{[0-9]+}', getUser, async c => {
    ...
    const result = await db.select().from(petsTable).where(eq(petsTable.id, paramId));
    const { id, name, species, breed, dateOfBirth, weight, ownerId, imageUrl } = result[0];
  ...
  .delete('/:id{[0-9]+}', getUser, async c => {
    ...
    const petToDelete = await db.select().from(petsTable).where(eq(petsTable.id, paramId));
  ...
  .get('/all-species', getUser, async c => {
    try {
      ...
      return c.json({ success: false, error: 'Failed to fetch all species' }, 500);
    }
  })
  .get('/:id{[0-9]+}/health-records', getUser, async c => {
    ...
    const recordsResults = await db
      ...
      .leftJoin(vetContactsTable, eq(healthRecordsTable.vetId, vetContactsTable.id));
  ...
  .get('/:id{[0-9]+}/caregivers', getUser, async c => {
    ...
    const vetResults = await db
      ...
      .where(eq(healthRecordsTable.petId, paramId))
      .leftJoin(healthRecordsTable, eq(vetContactsTable.id, healthRecordsTable.vetId));
