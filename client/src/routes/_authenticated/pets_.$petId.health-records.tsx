...
import { PlusCircle, FileText, Edit, TrashIcon, MapPin, Phone, Mail } from 'lucide-react';
...
export const Route = createFileRoute('/_authenticated/pets_/$petId/health-records')({
...
function HealthRecords() {
  ...
  if (error)
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-destructive">Error loading health records: {error.message}</div>
      </div>
    );
  ...
  if (!data.records || data.records.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-screen gap-4">
        <p className="text-muted-foreground">No health records found for this pet</p>
        <Button>
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="space-y-4">
        {data.records.map(record => (
          <Card
            key={record.id}
            className="shadow-sm border-0 bg-white/50 dark:bg-gray-800/80 dark:backdrop-blur-sm"
          >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-semibold">
                    Visit on {format(new Date(record.visitDate), 'MMM dd, yyyy')}
                  </h3>
                  <p className="text-sm text-muted-foreground">{record.vet.clinic}</p>
                </div>
              </div>
